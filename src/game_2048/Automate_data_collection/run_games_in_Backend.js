...
const NUM_WORKERS = 8;
const GAMES_PER_WORKER = 1000;
const outputFile = "gamePlayData/training_data.ndjson";
      ...
      worker.on('message', (message) => {
        if (message.type === 'log') {
          ...
          if (completedWorkers === NUM_WORKERS) {
            writeStream.end();
            console.log(`Saved training data in ${outputFile}`);
            resolve();
          }
        }
      });
...
//             fs.writeFileSync('gamePlayData/training_data.json', JSON.stringify(allResults));
